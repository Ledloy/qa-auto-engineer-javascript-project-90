export class TasksPage {
  ...
  async clearFilters() {
    await this.assigneeFilterSelect.click();
    const assigneeFirstOption = this.page.getByRole('option').first();
    await assigneeFirstOption.waitFor({ state: 'visible', timeout: 5000 });
    await assigneeFirstOption.click();
  
    await this.assigneeFilterSelect.waitFor({ 
      state: 'visible', 
      timeout: 5000 
    });
    
    await this.statusFilterSelect.click();
    const statusFirstOption = this.page.getByRole('option').first();
    await statusFirstOption.waitFor({ state: 'visible', timeout: 5000 });
    await statusFirstOption.click();
    
    await this.statusFilterSelect.waitFor({ 
      state: 'visible', 
      timeout: 5000 
    });
   
    await this.labelSelect.click();
    const labelFirstOption = this.page.getByRole('option').first();
    await labelFirstOption.waitFor({ state: 'visible', timeout: 5000 });
    await labelFirstOption.click();
    
    await this.kanbanBoard.waitFor({ 
      state: 'visible', 
      timeout: 5000 
    });
  }
